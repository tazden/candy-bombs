...
import com.example.candybasket.databinding.FragmentGameBinding
...
class GameFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.ivItem40.setOnClickListener {
            checkClick(Constants.FIFVE,Constants.ZERO)
        }
        binding.ivItem41.setOnClickListener {
            checkClick(Constants.FIFVE,Constants.ONE)
        }
        binding.ivItem42.setOnClickListener {
            checkClick(Constants.FIFVE, Constants.TWO)
        }
        binding.ivItem43.setOnClickListener {
            checkClick(Constants.FIFVE, Constants.THREE)
        }
    ...
    private fun checkClick(i: Int, j: Int) {
        if (coordinate != Constants.MINUS_ONE to Constants.MINUS_ONE && coordinate != i to j) {
    ...
    private fun checkGameOver() {
        var list = mutableListOf(Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO, Constants.ZERO,Constants.ZERO)
        var maxValue =Constants.ZERO

        for (i in Constants.ONE..Constants.SEVEN)
            for (j in Constants.ZERO..Constants.THREE) {
                list[Constants.listOfItems[i][j]] += Constants.ONE
                if (Constants.listOfItems[i][j] != Constants.TEN)
